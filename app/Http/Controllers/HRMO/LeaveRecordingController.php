...
class LeaveRecordingController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!$vlType || !$slType) {
            // Fallback: if not found, use any leave types or show error
            // For demo, we'll just use the first two or create dummy.
            $leaveTypes = LeaveType::where('status', true)->limit(2)->get();
            $vlType = $leaveTypes->first();
            $slType = $leaveTypes->skip(1)->first();
        }

        $leaveTypeIds = [$vlType->id ?? null, $slType->id ?? null];
        $leaveTypeNames = [
            $vlType->id => 'Vacation Leave',
            $slType->id => 'Sick Leave',
        ];
        ...
        if ($employeeId) {
            ...
            for ($m = 1; $m <= 12; $m++) {
                $months[$m] = [
                    ...
                    'transactions' => [],
                    'opening_balances' => $startBalances, // will be updated per month
                    'net_vl' => 0,
                    'net_sl' => 0,
                    'ending_balances' => [],
                    'movement_count' => 0,
            ...
            // Now compute opening balances for each month sequentially
            ...
            foreach ($months as $m => &$monthData) {
                ...
                // Process transactions to compute net and ending
                $netVl = 0;
                $netSl = 0;
