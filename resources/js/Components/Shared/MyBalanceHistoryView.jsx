...
const TYPE_META = {
    INITIAL_BALANCE:  { icon: Flag,          label: 'Initial Balance',  cls: 'bg-blue-50 text-blue-700 border-blue-200' },
    HR_ADJUSTMENT:    { icon: Settings2,     label: 'HR Adjustment',    cls: 'bg-amber-50 text-amber-700 border-amber-200' },
    MONTHLY_ACCRUAL:  { icon: TrendingUp,    label: 'Monthly Accrual',  cls: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    LEAVE_DEDUCTION:  { icon: TrendingDown,  label: 'Leave Deduction',  cls: 'bg-red-50 text-red-700 border-red-200' },
    LEAVE_RETURN:     { icon: RotateCcw,     label: 'Leave Return',     cls: 'bg-violet-50 text-violet-700 border-violet-200' },
    FINAL_ACCRUAL:    { icon: Flag,          label: 'Final Accrual',    cls: 'bg-orange-50 text-orange-700 border-orange-200' },
};
            ...
            >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                    <div className="min-w-0 flex-1">
                        ...
                        {/* Leave request details if present */}
                        {tx.leave_request && (
                            <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-gray-600">
                                <span className="inline-flex items-center gap-1">
                                    <FileText className="w-3 h-3 text-gray-400" />
                                    {fmtDate(tx.leave_request.start_date)}
                                    {tx.leave_request.end_date && tx.leave_request.end_date !== tx.leave_request.start_date && (
                                        <> – {fmtDate(tx.leave_request.end_date)}</>
                                    )}
                                </span>
                                <span className="text-gray-300">·</span>
                                <span className="tabular-nums">
                                    {tx.leave_request.days} day{tx.leave_request.days === 1 ? '' : 's'}
                                </span>
                            </div>
                        )}
...
// ---------- Main view ----------
export default function MyBalanceHistoryView({
    leaveType,
    transactions = [],
    currentBalance = 0,
    employee = {},
    ...
    // Always render newest-first, regardless of backend order.
    // Primary sort: transaction_date DESC.
    // Tiebreaker: id DESC (so same-day entries stay in creation order).
                ...
                >
                    ...
                    <div className="h-2 rounded-full bg-gray-100 overflow-hidden">
                        <div
                            ...
                            style={{ width: `${ratio}%`, backgroundColor: GOLD }}
                        />
                    </div>
                </div>
            )}
