...
export default function SpecialRequestInfo({ leaveRequest, balance, balanceLabel }) {
    ...
    if (isMonetization) {
        ...
        const total = balance || lr.number_of_days || 0;
        details.push({ label: 'Total Days (VL + SL)', value: `${formatDays(total)} days` });
        // Add breakdown if we have separate VL and SL
        // We need to pass vlBalance and slBalance from controller
        // For now, we can attempt to show a breakdown if we have the data
        // Or we can just show the total and note it's VL+SL
        details.push({ label: 'Reason', value: lr.reason || '—' });
    }
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                        {isMonetization
                            ? 'This is a monetization request for Vacation Leave credits.'
                            : 'This is a Terminal Leave request that uses both VL and SL balances.'}
                    </p>
                </div>
            </div>
        </div>
