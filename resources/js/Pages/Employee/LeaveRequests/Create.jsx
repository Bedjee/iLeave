...
// ============================================================
    ...
    if (selectedType.name.toLowerCase().includes('maternity')) {
     ...
     // 9. Adoption Leave
        ...
        specialType,
         vlBalance,
        // data.monetized_days is already included via `data` object
    ]);
...
const countWorkingDays = (dateStrings) => {
  ...
  useEffect(() => {
    ...
    if (selectedType.date_selection_type === 'specific_dates') {
        const validDates = data.dates.filter(d => d && d.trim() !== '');
        let days = validDates.length;
        // For Sick Leave, count only working days
        if (selectedType.name.toLowerCase().includes('sick')) {
            days = countWorkingDays(validDates);
        }
   ...
   const { frontendErrors, isValid } = useLeaveValidation(
        ...
        calculatedDays,
        balances,
        hasTakenMaternityLeave ,// new
          hasTakenAdoptionLeave,
        !!selectedSpecialType, // isSpecialRequest
...
const handleFileChange = (e) => {
    ...
    if (file.type.startsWith('image/')) {
        ...
        // For non-image files, just set the file without preview
        setPreviewUrl(null);
        setData('attachment', file);
    }
};
...
<div className="mt-6">
    ...
    {/* Show a hint when a special request is selected */}
    {selectedSpecialType && (
    ...
    <div className="mt-4">
        ...
        />
                    ...
                    <form onSubmit={submit} className="mt-6 space-y-4">
                        ...
                        {selectedType?.name.toLowerCase().includes('wellness') && (
                            (() => {
                                const wellnessKeys = ['wellness_consecutive', 'wellness_split', 'wellness_gap', 'wellness_total'];
                                const wellnessError = wellnessKeys.reduce((acc, key) => acc || frontendErrors[key], null);
                                if (!wellnessError) return null;

                                let detailedHelp = '';
                                if (frontendErrors.wellness_consecutive) {
                                    detailedHelp = 'Wellness Leave cannot be taken in blocks longer than 3 consecutive days. Please split your leave into smaller blocks.';
                                } else if (frontendErrors.wellness_split) {
                                    detailedHelp = 'If you take 5 days of Wellness Leave, they must be split into two separate blocks: one of 3 days and one of 2 days, with at least one day between them.';
                                } else if (frontendErrors.wellness_gap) {
                                    detailedHelp = 'The two blocks of Wellness Leave must be separated by at least one day. Please add a gap between the blocks.';
                                } else if (frontendErrors.wellness_total) {
                                    detailedHelp = 'You can only take up to 5 days of Wellness Leave. You selected more than 5 days.';
                                }

                                return (
                                    <div className="p-4 bg-red-50 border-l-4 border-red-500 rounded-md">
                                        <div className="flex items-start gap-2">
                                            <svg className="size-5 text-red-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                            </svg>
                                            <div>
                                                <h4 className="text-sm font-semibold text-red-800">Wellness Leave Restriction</h4>
                                                <p className="text-sm text-red-700 mt-1">{detailedHelp}</p>
                                                <p className="text-xs text-red-600 mt-1">
                                                    <span className="font-medium">Rule:</span> Wellness Leave has a maximum of 5 days, with no more than 3 consecutive days. If using all 5 days, split them into blocks of 3 and 2 with at least one day gap.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                       ...
                       {selectedType && selectedType.date_selection_type === 'specific_dates' && (
    <div className="p-3 bg-gray-50 border border-gray-200 rounded-md">
        <p className="text-sm text-gray-700">
            <span className="font-medium">Inclusive Dates:</span> {data.dates.filter(d => d && d.trim() !== '').length} days
        </p>
        <p className="text-sm text-gray-700">
            <span className="font-medium">Working Days:</span> {calculatedDays} days
            {selectedType.name.toLowerCase().includes('sick') && (
                <span className="text-xs text-gray-500 ml-2">(weekends excluded for balance deduction)</span>
            )}
        </p>
    </div>
)}
