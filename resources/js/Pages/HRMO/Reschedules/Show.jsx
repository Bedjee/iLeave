...
export default function Show({ reschedule }) {
    ...
    const renderDateGroup = (datesArray) => {
        if (!datesArray || datesArray.length === 0) return <span className="text-sm text-gray-400">—</span>;
        const grouped = groupDatesByMonth(datesArray);
        if (grouped.length === 0) return <span className="text-sm text-gray-400">—</span>;
    ...
    // --- Handlers ---
    const handleCertify = () => {
        if (!confirm('Are you sure you want to certify this reschedule request?')) return;
        setProcessing(true);
        router.post(route('hrmo.reschedules.certify', r.id), {}, {
            onSuccess: () => setProcessing(false),
            onError: () => { setProcessing(false); alert('Action failed.'); },
        });
    };
    ...
    const confirmReject = () => {
        ...
        setProcessing(true);
        router.post(route('hrmo.reschedules.reject', r.id), { reason: rejectReason }, {
            onSuccess: () => {
    ...
    // --- Date displays ---
    const originalDates = r.old_dates && r.old_dates.length > 0
        ...
        <HRMOLayout>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
                ...
                <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 mb-6 text-sm text-gray-600">
                    {r.status === 'pending' && 'This reschedule request is awaiting review. You can certify or reject it.'}
                    {r.status === 'certified' && 'This reschedule has been certified by HRMO and is awaiting Department Head approval.'}
                    {r.status === 'approved' && 'This reschedule request has been fully approved. The leave dates have been updated.'}
                    {r.status === 'rejected' && 'This reschedule request has been rejected.'}
                </div>

                {/* Main Card */}
                <div className="bg-white rounded-xl border p-5 sm:p-6 shadow-sm space-y-6" style={{ borderColor: 'rgba(15,42,82,0.08)' }}>

                    {/* Employee & Leave Type */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <p className="text-xs font-medium uppercase tracking-wider" style={{ color: NAVY }}>Employee</p>
                            <p className="text-base font-semibold text-gray-900">{employee.full_name || '—'}</p>
                            <p className="text-sm text-gray-600">{employee.position || '—'}</p>
                            <p className="text-sm text-gray-600">{department.department_name || '—'}</p>
                        </div>
                        <div>
                            <p className="text-xs font-medium uppercase tracking-wider" style={{ color: NAVY }}>Leave Type</p>
                            <p className="text-base font-semibold text-gray-900">{leaveType.name || '—'}</p>
                            <p className="text-sm text-gray-600">Original Request #{leaveRequest.id || '—'}</p>
                        </div>
                    </div>

                    {/* Day Summary */}
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 rounded-xl p-4 border" style={{ backgroundColor: NAVY_LIGHT, borderColor: 'rgba(15,42,82,0.1)' }}>
                        <div>
                            <p className="text-xs text-gray-500">Original Days</p>
                            <p className="text-lg font-bold text-gray-900">{formatDays(r.old_number_of_days)}</p>
                        </div>
                        <div>
                            <p className="text-xs text-gray-500">New Days</p>
                            <p className="text-lg font-bold text-gray-900">{formatDays(r.new_number_of_days)}</p>
                        </div>
                        <div>
                    ...
                    {/* Date Change */}
                    <div>
                        <h3 className="text-xs font-medium uppercase tracking-wider mb-3" style={{ color: NAVY }}>Date Change</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {r.reason && (
                        <div>
                            <h3 className="text-xs font-medium uppercase tracking-wider" style={{ color: NAVY }}>Reason</h3>
                    ...
                    {r.attachment && (
                        <div>
                            <h3 className="text-xs font-medium uppercase tracking-wider" style={{ color: NAVY }}>Attachment</h3>
                            <a
                                href={r.attachment}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1.5 mt-1 text-sm text-[#ffbf00] hover:underline font-medium"
                            >
                ...
                {/* Timeline */}
                {events.length > 0 && (
                    <div className="bg-white rounded-xl border p-5 sm:p-6 shadow-sm mt-6" style={{ borderColor: 'rgba(15,42,82,0.08)' }}>
                        <h3 className="text-xs font-medium uppercase tracking-wider mb-4" style={{ color: NAVY }}>Workflow Timeline</h3>
                ...
                {/* Actions */}
                {canAct && (
                    <div className="bg-white rounded-xl border p-5 shadow-sm mt-6" style={{ borderColor: 'rgba(15,42,82,0.08)' }}>
                        <h3 className="text-xs font-medium uppercase tracking-wider mb-3" style={{ color: NAVY }}>Actions</h3>
                        <div className="flex flex-wrap gap-3">
                            <button
                                onClick={handleCertify}
                                disabled={processing}
                                className="inline-flex items-center gap-2 px-5 py-2.5 text-white rounded-lg transition text-sm font-medium disabled:opacity-50"
                        ...
                        <p className="text-xs text-gray-500 mt-3">
                            Certify to recommend approval, or reject to decline this reschedule request.
                        </p>
                    </div>
                )}
