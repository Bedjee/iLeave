...
export default function Show({ leaveRequest }) {
    ...
    // --- Workflow Timeline for Department Head's own requests ---
    // DH → HRMO → Admin (DH approval is skipped)
    const steps = [
        { key: 'submitted', label: 'Filed by Department Head', status: lr.submitted_at || lr.created_at ? 'completed' : 'pending' },
        { key: 'certified', label: 'HRMO Certified', status: lr.certified_at ? 'completed' : lr.status === 'certified' ? 'current' : 'pending' },
        { key: 'department_approved', label: 'Department Approved (Auto)', status: lr.status === 'department_approved' || lr.final_approved_at ? 'completed' : lr.status === 'department_approved' ? 'current' : 'pending' },
        { key: 'approved', label: 'Final Approval (Admin/Mayor)', status: lr.status === 'approved' ? 'completed' : 'pending' },
    ];
    ...
    return (
        <DepartmentHeadLayout>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
                <Link href={route('department-head.my-leave-requests.index')} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition mb-6">
                ...
                {/* ===== STATUS HERO BANNER – LARGER & PROMINENT ===== */}
                <div className="bg-white rounded-2xl shadow-md border border-gray-200 p-6 sm:p-8 mb-8 relative overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <div className={`flex items-center justify-center w-14 h-14 rounded-2xl ${getStatusBadge(lr.status)} text-2xl font-bold flex-shrink-0`}>
                                ...
                                {lr.status === 'mayor_pending' && (
                                    <svg className="size-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                )}
                                {!['approved','rejected','mayor_pending'].includes(lr.status) && (
                                    <svg className="size-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                )}
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm">
                            {isRejected && lr.rejection_reason && (
                                <span className="px-3 py-1.5 bg-red-50 border border-red-200 rounded-lg text-red-700">
                                    Reason: {lr.rejection_reason}
                                </span>
                            )}
                            {isCancelled && lr.cancellation_reason && (
                                <span className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-700">
                                    Reason: {lr.cancellation_reason}
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-4 text-sm text-gray-600">
                        ...
                        {lr.approved_at && (
                            <div>
                                <span className="font-medium">Department Approved:</span> {formatDateTime(lr.approved_at)}
                            </div>
                        )}
                        {lr.final_approved_at && (
                            <div>
                                <span className="font-medium">Final Approved:</span> {formatDateTime(lr.final_approved_at)}
                            </div>
                        )}
                        {lr.rejected_at && (
                            <div>
                                        ...
                                        return (
                                            <div key={step.key} className="relative flex gap-4 pb-6 last:pb-0">
                                                ...
                                                <div className="flex-1 pt-1">
                                                    ...
                                                    {isCompleted && step.key === 'department_approved' && lr.approved_at && (
                                                        <p className="text-xs text-gray-500 mt-0.5">{formatDateTime(lr.approved_at)}</p>
                                                    )}
                                                    {isCompleted && step.key === 'approved' && lr.final_approved_at && (
                                                        <p className="text-xs text-gray-500 mt-0.5">{formatDateTime(lr.final_approved_at)}</p>
                                                    )}
                                                </div>
                                            </div>
                                        );
