            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center gap-3">
                        <div className="flex size-10 sm:size-12 items-center justify-center rounded-full" style={{ backgroundColor: 'rgba(255,191,0,0.15)', color: GOLD }}>
                            <svg className="size-5 sm:size-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                        </div>
                        <div>
                    ...
                    <Link
                        href={route('department-head.my-leave-requests.create')}
                ...
                {leaveRequests.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border p-8 text-center" style={{ borderColor: 'rgba(15,42,82,0.08)' }}>
                        ...
                        <Link href={route('department-head.my-leave-requests.create')} className="inline-block mt-3 text-sm font-medium hover:underline" style={{ color: GOLD }}>File your first leave request</Link>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {leaveRequests.map((req) => {
                            ...
                            return (
                                <div key={req.id} className={`bg-white rounded-xl shadow-sm border ${showLeftBorder ? 'border-l-4' : 'border-gray-200'} ${approvedRescheduleClass} p-4 hover:shadow-md transition-all duration-200 group relative`} style={showLeftBorder ? { borderColor: GOLD } : {}}>
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            {rescheduleBadge && <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${rescheduleBadge.color}`}>{rescheduleBadge.icon}{rescheduleBadge.label}</span>}
                                        </div>
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadge(req.status, hasApprovedReschedule)} self-start sm:self-center`}>{getStatusLabel(req.status, hasApprovedReschedule)}</span>
                                    </div>
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-xs text-gray-500 truncate">
                                                    {req.start_date && req.end_date ? `${formatDate(req.start_date)} – ${formatDate(req.end_date)}` : req.dates?.map(d => formatDate(d.leave_date)).join(', ') || '—'}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3 self-start sm:self-center flex-shrink-0">
                                            <span className="text-xs text-gray-400 whitespace-nowrap">{req.number_of_days} day{req.number_of_days !== 1 ? 's' : ''}</span>
                                        </div>
                                    </div>
                                    <div className="mt-3 pt-3 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                                        <div className="w-full min-w-0 flex-1"><LeaveRequestProgress status={req.status} workflow="department_head" /></div>
                                        <div className="flex items-center gap-3 self-end sm:self-center">
                                            {hasActiveReschedule && <Link href={route('employee.reschedules.show', req.active_reschedule.id)} className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: GOLD }}>View Reschedule</Link>}
                                            <Link href={route('department-head.my-leave-requests.show', req.id)} className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: GOLD }}>Details</Link>
                                        </div>
                                    </div>
                                </div>
                            );
