...
export default function Show({ leaveRequest }) {
    ...
    // --- Workflow Timeline for Admin Leave Requests ---
    const steps = [
        { key: 'submitted', label: 'Filed by Admin', status: lr.submitted_at || lr.created_at ? 'completed' : 'pending' },
        { key: 'certified', label: 'HRMO Certified', status: lr.certified_at ? 'completed' : lr.status === 'certified' ? 'current' : 'pending' },
        { key: 'mayor', label: 'Mayor Final Approval', status: lr.final_approved_at ? 'completed' : lr.status === 'mayor_pending' ? 'current' : 'pending' },
        { key: 'approved', label: 'Approved', status: lr.status === 'approved' ? 'completed' : 'pending' },
    ];
    ...
    return (
        <AdminLayout>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
                <Link href={route('admin.my-leave-requests.index')} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition mb-6">
                                        ...
                                        return (
                                            <div key={step.key} className="relative flex gap-4 pb-6 last:pb-0">
                                                ...
                                                <div className="flex-1 pt-1">
                                                    ...
                                                    {isCompleted && step.key === 'mayor' && lr.final_approved_at && (
