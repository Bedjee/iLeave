        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <Link
                        href={route('admin.my-leave-requests.create')}
                ...
                {leaveRequests.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border p-8 text-center" style={{ borderColor: 'rgba(15,42,82,0.08)' }}>
                        ...
                        <Link href={route('admin.my-leave-requests.create')} className="inline-block mt-3 text-sm font-medium hover:underline" style={{ color: GOLD }}>File your first leave request</Link>
                    </div>
                ) : (
                            ...
                            return (
                                <div key={req.id} className={`bg-white rounded-xl shadow-sm border ${showLeftBorder ? 'border-l-4' : 'border-gray-200'} ${approvedRescheduleClass} p-4 hover:shadow-md transition-all duration-200 group relative`} style={showLeftBorder ? { borderColor: GOLD } : {}}>
                                    ...
                                    <div className="mt-3 pt-3 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                                        <div className="w-full min-w-0 flex-1"><LeaveRequestProgress status={req.status} workflow="admin" /></div>
                                        <div className="flex items-center gap-3 self-end sm:self-center">
                                            ...
                                            <Link href={route('admin.my-leave-requests.show', req.id)} className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: GOLD }}>Details</Link>
                                        </div>
                                    </div>
                                </div>
                            );
