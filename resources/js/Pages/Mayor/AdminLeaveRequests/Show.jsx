...
export default function Show({ leaveRequest, balance }) {
    ...
    const getStatusInfo = (status) => {
        ...
        return map[status] || map.mayor_pending;
    };
    ...
    if (lr.final_approved_at) {
        events.push({ label: 'Approved by Mayor', date: lr.final_approved_at, icon: 'approved' });
    }
    if (lr.rejected_at) {
    ...
    const getTimelineColor = (label) => {
        const map = {
            ...
            'Approved by Mayor': 'bg-green-500',
            Rejected: 'bg-red-500',
        };
    ...
    const renderDateDisplay = () => {
        if (lr.dates && lr.dates.length > 0) {
            ...
            return <span className="text-sm text-gray-700">{formatDate(lr.start_date)} – {formatDate(lr.end_date)}</span>;
        } else {
    ...
    // --- Actions ---
    const handleApprove = () => {
        if (!confirm('Are you sure you want to approve this admin leave request?')) return;
        setProcessing(true);
        router.post(route('mayor.admin-leave-requests.approve', lr.id), {}, {
    ...
    const confirmReject = () => {
        ...
        setProcessing(true);
        router.post(route('mayor.admin-leave-requests.reject', lr.id), {
            reason: rejectReason,
        }, {
            onSuccess: () => {
    ...
    const isActionable = lr.status === 'mayor_pending';

    return (
        <MayorLayout>
            <Head title={`Admin Leave Request #${lr.id}`} />
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
                ...
                <div className={`rounded-xl border ${statusInfo.bg} ${statusInfo.border} p-4 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3`}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">Admin Leave Request</p>
                        </div>
                    </div>
                ...
                {!isActionable && lr.status !== 'mayor_pending' && (
