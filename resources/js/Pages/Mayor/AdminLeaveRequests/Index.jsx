...
export default function Index({ pendingRequests = [], approvedRequests = [] }) {
    ...
    const getCurrentRequests = () => {
        let requests = activeTab === 'pending' ? pendingRequests : approvedRequests;
        ...
        <MayorLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    <div>
                        <h2 className="text-2xl font-bold flex items-center gap-2" style={{ color: DEEP_RED }}>
                            <svg className="size-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: PRIMARY_RED }}>
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Admin Leave Requests
                        </h2>
                        <p className="text-sm text-gray-500">Review and approve leave requests filed by Admins.</p>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                        ...
                        <span className="text-gray-500">{totalPending + totalApproved} total</span>
                    </div>
                </div>
                            ...
                            {searchTerm ? (
                                ...
                                <>
                                    <p className="text-gray-500 text-sm">
                                        {activeTab === 'pending' ? 'No admin leave requests pending your approval.' : 'No approved admin leave requests yet.'}
                                    </p>
                                    <p className="text-gray-400 text-xs mt-1">
                                        {activeTab === 'pending' ? 'Requests certified by HRMO will appear here.' : 'Approved requests will be listed here.'}
                                    </p>
                                </>
                            )}
                        </div>
                ...
                {currentRequests.length > 0 && (
                    <div className="mt-4 text-xs text-gray-400 text-right">
                        Showing {displayedCount} of {totalCount} {activeTab === 'pending' ? 'pending' : 'approved'} requests
                        {searchTerm && ' (filtered)'}
                    </div>
                )}
            </div>
                        ...
                        <>
                            <button
                                onClick={() => {
                                    ...
                                    if (confirm('Are you sure you want to approve this admin leave request?')) {
                                        router.post(route('mayor.admin-leave-requests.approve', openDropdownId), {}, {
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    if (reason !== null) {
                                        router.post(route('mayor.admin-leave-requests.reject', openDropdownId), { reason }, {
                                            onSuccess: () => router.reload(),
